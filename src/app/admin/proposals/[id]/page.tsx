    ...
    <div style={{ minHeight: '100vh', background: '#f1f5f9', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ flex: 1, maxWidth: '960px', width: '100%', margin: '0 auto', padding: '2rem 1.25rem 4rem', boxSizing: 'border-box' }}>
        ...
        >
          ...
          {/* ═══════════════ LỊCH TRÌNH CHI TIẾT CÁC CA / BUỔI & ĐỊA ĐIỂM MƯỢN ═══════════════ */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
              <h3 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#0f172a', margin: 0, display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                <ClockIcon size={20} color="#7e22ce" />
                <span>Lịch Trình Chi Tiết Các Ca / Buổi & Địa Điểm Mượn:</span>
              </h3>
              <span style={{ fontSize: '0.8rem', fontWeight: 700, background: '#f3e8ff', color: '#7e22ce', padding: '0.2rem 0.6rem', borderRadius: '12px' }}>
                {(proposal.sessions?.length || 1)} ca hoạt động
              </span>
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
              {(proposal.sessions && proposal.sessions.length > 0 ? proposal.sessions : [
                {
                  id: 'session_1',
                  name: 'Buổi chính / Khai mạc',
                  session_date: proposal.start_date,
                  start_time: proposal.start_time,
                  end_time: proposal.end_time,
                  room_name: proposal.room_name,
                  participant_count: proposal.participant_count,
                  purpose: proposal.description || 'Hoạt động sự kiện',
                  status: 'pending',
                }
              ]).map((sess: any, idx: number) => {
                const decision = sessionDecisions[sess.id];
                const currentSessionStatus = decision?.status || sess.status || 'pending';
                const currentReason = decision?.rejection_reason || sess.rejection_reason;

                return (
                  <div
                    key={sess.id || idx}
                    style={{
                      background: currentSessionStatus === 'approved' ? '#f0fdf4' : currentSessionStatus === 'rejected' ? '#fef2f2' : '#f8fafc',
                      border: currentSessionStatus === 'approved' ? '1.5px solid #86efac' : currentSessionStatus === 'rejected' ? '1.5px solid #fca5a5' : '1.5px solid #e2e8f0',
                      borderRadius: '16px',
                      padding: '1.25rem',
                      display: 'flex',
                      flexDirection: 'column',
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <span style={{ width: '26px', height: '26px', borderRadius: '50%', background: '#7e22ce', color: '#ffffff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', fontWeight: 800 }}>
                          {idx + 1}
                        </span>
                        <strong style={{ fontSize: '1rem', color: '#0f172a' }}>{sess.name}</strong>
                      </div>

                      {/* Status badge */}
                      {currentSessionStatus === 'approved' ? (
                        <span style={{ fontSize: '0.75rem', fontWeight: 800, color: '#166534', background: '#dcfce7', border: '1px solid #bbf7d0', padding: '0.2rem 0.6rem', borderRadius: '12px' }}>
                          ✓ Đã chấp thuận ca này
                        </span>
                      ) : currentSessionStatus === 'rejected' ? (
                        <span style={{ fontSize: '0.75rem', fontWeight: 800, color: '#991b1b', background: '#fee2e2', border: '1px solid #fecaca', padding: '0.2rem 0.6rem', borderRadius: '12px' }}>
                          ✕ Từ chối ca này {currentReason ? `(${currentReason})` : ''}
                        </span>
                      ) : (
                        <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#b45309', background: '#fef3c7', border: '1px solid #fde68a', padding: '0.2rem 0.6rem', borderRadius: '12px' }}>
                          ⏳ Chờ thẩm định
                        </span>
                      )}
                    </div>

                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.6rem', fontSize: '0.85rem', color: '#475569' }}>
                      <div>📅 <strong>Ngày:</strong> {sess.session_date ? new Date(sess.session_date).toLocaleDateString('vi-VN') : 'Chưa chọn'}</div>
                      <div>⏰ <strong>Giờ:</strong> {sess.start_time || '08:00'} - {sess.end_time || '11:30'}</div>
                      <div>🏢 <strong>Địa điểm:</strong> <strong style={{ color: sess.room_name && sess.room_name !== 'Không mượn' ? '#7e22ce' : '#64748b' }}>{sess.room_name || 'Không mượn phòng'}</strong></div>
                      <div>👥 <strong>Quy mô:</strong> {sess.participant_count || proposal.participant_count} người</div>
                    </div>

                    {sess.purpose && (
                      <div style={{ fontSize: '0.825rem', color: '#64748b', background: '#ffffff', padding: '0.5rem 0.75rem', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                        📝 <i>Mục đích: {sess.purpose}</i>
                      </div>
                    )}

                    {/* Approver Granular Controls */}
                    {canActOnThisStage && proposal.status === 'pending' && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.25rem', paddingTop: '0.65rem', borderTop: '1px solid #e2e8f0', flexWrap: 'wrap' }}>
                        <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#334155' }}>Duyệt riêng ca này:</span>
                        <button
                          type="button"
                          onClick={() => setSessionDecisions((prev) => ({ ...prev, [sess.id]: { status: 'approved' } }))}
                          style={{
                            padding: '0.25rem 0.65rem',
                            borderRadius: '6px',
                            border: '1px solid #86efac',
                            background: currentSessionStatus === 'approved' ? '#16a34a' : '#f0fdf4',
                            color: currentSessionStatus === 'approved' ? '#ffffff' : '#166534',
                            fontSize: '0.75rem',
                            fontWeight: 700,
                            cursor: 'pointer',
                          }}
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            const reason = prompt('Nhập lý do từ chối riêng ca này (VD: Trùng lịch hội trường):', 'Trùng lịch phòng');
                            if (reason !== null) {
                              setSessionDecisions((prev) => ({ ...prev, [sess.id]: { status: 'rejected', rejection_reason: reason } }));
                            }
                          }}
                          style={{
                            padding: '0.25rem 0.65rem',
                            ...
                            background: currentSessionStatus === 'rejected' ? '#dc2626' : '#fef2f2',
                            color: currentSessionStatus === 'rejected' ? '#ffffff' : '#991b1b',
                            fontSize: '0.75rem',
                            fontWeight: 700,
                            cursor: 'pointer',
                          }}
